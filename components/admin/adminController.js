...
module.exports.updateInfo = async (req, res, next) =>{
    try {
        ...
        await adminService.updateUser(req.body.id,req.body.name,req.body.phone);
        const user = await adminService.getUserByID(req.body.id);
        console.log(user);
...
module.exports.createAdmin = async (req, res, next) =>{
    try {
        const admin = await adminService.getUserByUsername(req.body.username);
        if(admin){
            res.status(401).json({message:"user_exists"});
        }else{
            const newAdmin = await adminService.createAdmin(req.body.username, req.body.phone, req.body.name, req.body.phone, req.body.email, "2");
